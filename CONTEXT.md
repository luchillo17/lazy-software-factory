...
**Host sandbox**:
A `SandboxProvider` backend where the sandbox **is** the host process/filesystem (local `exec` + Host-launched **ADW worker**). Lasting option for single-ADW-at-a-time local use without containers (`adw --sandbox host`, Factory **`adw-host`** bin, or `pnpm adw:host`). Not a second orchestration path — same worker protocol and warm-sandbox rules, weaker isolation. Operator may aim the warm sandbox at a named git tree via **`--cwd` / `ADW_CWD`** or `adw-host` (invoker cwd when `--cwd` omitted) — still Host, not Docker (VISION §4 / ADR-0015 / ADR-0016).
...
**Docker sandbox**:
Classic Docker Engine adapter: one hardened container + ephemeral named workspace volume per lease; remote Git source only (no host cwd bind mounts); worker protocol via `docker exec` after a handshake probe. Configurable concurrent allocation (typed capacity error, no internal queue); CPU/memory/PID/lifetime enforced and reported where Docker supports them. **Default** for generic `adw` after live proof (#86); Host remains available via `--sandbox host` / `adw-host`. Provider-semantic map for future Vercel: `docs/agents/sandbox-provider-conformance.md`. Operator: `docs/docker-operator.md`. Sandbox isolates compute/filesystem, **not** shared external backends (forges, cloud deploys, SaaS).
_Avoid_: Publishing inbound ports; mounting the Docker socket; privileged mode; baking run secrets into image layers or `docker create` env; claiming retained workspaces or disk quotas in Docker v1; provider-side scheduling/queueing; assuming sandbox isolation covers shared external backends
...
**Workspace provision**:
Deterministic ADW setup inside the warm sandbox **before** Build: ensure a git worktree exists, create/checkout the ticket branch (orchestration-owned, e.g. `adw/<ticketId>`), then run a **locked install** for generic Node repos (npm / pnpm / Yarn via lockfile + optional `packageManager` / Corepack; Bun and non-Node need another runner). **Host:** reuse an already-cloned path when `.git` is present (skip clone; still branch + install as needed). **Cloud/empty box:** Git host clone (+ install) using per-run credentials. Provision failure → ADW `failed` with no agent run. Not an LLM step; not Ship. Does not require Nx on the target.
_Avoid_: Assuming every sandbox already has the repo; making the Build agent own clone/branch/install; treating Host and cloud as different ADW graphs; requiring a custom bootstrap script in v0; expecting `--repo-url` to replace an existing Host `.git`; assuming the target is this Factory’s pnpm/Nx workspace
...
_Avoid_: Treating intake as a Minimal ADW graph step; full triage/grill productization; requiring paste-by-hand as the only Host path once the GitHub adapter exists; failing intake because GitHub `blocked_by > 0`; encoding wayfinder/stack dependency graphs as ADW routing
