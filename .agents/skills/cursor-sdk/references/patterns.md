...
async function main() {
  const { PR_URL, REPO_URL, HEAD_REF, BASE_REF, CURSOR_API_KEY, GITHUB_TOKEN } =
    process.env;
...
app.post("/agents/:userId/send", async (req, res) => {
  ...
  const options = {
    ...
    cloud: {
      repos: [
        { url: "https://github.com/your-org/workspace", startingRef: "main" },
      ],
    },
  };
...
const results = await Promise.allSettled(
  repos.map((r) => dispatchOne(r, apiKey, prompt))
);
